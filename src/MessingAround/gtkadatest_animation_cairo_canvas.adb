...
-- with GDK.Drawable;     use GDK.Drawable; --gone in gtk 3
...
package body GtkAdaTest_Animation_Cairo_Canvas is
...
--                            )
--                          );
      -- Context := Create( Gtk.Window.Gtk_Window( Get_Window( Buffer ) ) );
...
--                                 );
      ...
      Buffer.Surface := Create (Cairo_Format_RGB24,
                                GInt( Get_Allocated_Width(Buffer) ),
                                GInt( Get_Allocated_Height(Buffer) )
                               );
...
end GtkAdaTest_Animation_Cairo_Canvas;
